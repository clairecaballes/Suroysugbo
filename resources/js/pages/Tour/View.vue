  ...
  <Link href="/" class="inline-block px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">
    ...
    <div class="flex flex-col lg:flex-row gap-8 w-full max-w-6xl mx-auto p-4 lg:p-6 bg-white rounded-lg shadow-lg mt-4 lg:mt-7">
        ...
        <div class="flex flex-col items-center lg:items-start">
            <div class="w-full max-w-[300px] sm:max-w-[400px] h-[300px] sm:h-[400px] overflow-hidden rounded-xl bg-gray-100 flex items-center justify-center">
        ...
        <div class="flex-1 flex flex-col gap-6">
            <div>
                <h2 class="text-xl sm:text-2xl font-semibold mb-2 text-center lg:text-left">
            ...
            <div class="flex-1">
                ...
                <table class="w-full text-left border-collapse text-sm sm:text-base">
                    <thead>
                        <tr>
                            <th class="border-b-2 border-gray-300 px-2 sm:px-4 py-2">Jeepney Code</th>
                            <th class="border-b-2 border-gray-300 px-2 sm:px-4 py-2">Route Description</th>
                            <th class="border-b-2 border-gray-300 px-2 sm:px-4 py-2">Fare (₱)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(route, index) in props.legacyItem?.vehicle_routes || []" :key="index">
                            <td class="border-b border-gray-200 px-2 sm:px-4 py-2">{{ route.vehicle_code }}</td>
                            <td class="border-b border-gray-200 px-2 sm:px-4 py-2">{{ route.description }}</td>
                            <td class="border-b border-gray-200 px-2 sm:px-4 py-2">{{ route.price }}</td>
                        </tr>
                    </tbody>
                </table>
                <!-- Google Map View Placeholder -->
                <GoogleMap api-key="AIzaSyCVi0HNMRa3sByY_4O8cwrwF2D7DdVV2hA" mapId="DEMO_MAP_ID"
                    style="width: 100%; height: 300px; sm:height: 500px;" :center="center" :zoom="15">
                </GoogleMap>
            </div>
        </div>
